...
export class CuentasService {
  ...
  depositar(cuentaId: string, monto: string): Observable<any> {
    const payload = this.encryptPayload({
      cuentaId: cuentaId,
      monto: monto,
    });

    const bodyContent = `<ser:Depositar> <ser:cuentaId>${payload.cuentaId}</ser:cuentaId> <ser:monto>${payload.monto}</ser:monto> </ser:Depositar>`;
    ...
    return this.http
      ...
      .pipe(
        map((response) => {
          ...
          const depositarResult =
            xmlDoc.getElementsByTagName('DepositarResult')[0];
          if (!depositarResult) {
            throw new Error('Invalid XML response');
          }

          const dataElement = depositarResult.getElementsByTagName('Data')[0];
          if (!dataElement) {
            throw new Error('Invalid XML response');
          }

          const montoDepositado = this.encryptionService.decrypt(
            dataElement.getElementsByTagName('Monto')[0].textContent!
          );
          const saldoActual = this.encryptionService.decrypt(
            dataElement.getElementsByTagName('SaldoActual')[0].textContent!
          );
